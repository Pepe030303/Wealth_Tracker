...
-   **배당금 분석 페이지 (`/dividends`)**:
    -   **지급 완료/예정 배당 분리**: 현재 연도를 기준으로, 과거 월에 **지급 완료된 배당**과 미래 월에 **지급이 예정된 배당**을 명확히 구분하여 보여줍니다. 이를 통해 사용자는 실제 현금 흐름과 미래 예측을 한눈에 파악할 수 있습니다.
    -   **데이터 기반 시각화**: '지급 완료' 배당은 DB에 기록된 실제 수령액을, '지급 예정' 배당은 API를 통해 확인된 공식 발표 데이터를 기반으로 하므로 매우 정확합니다. 월별 차트는 두 유형의 배당을 각기 다른 색상의 스택(stacked) 그래프로 시각화합니다.
    -   **인터랙티브 차트**: 월별 배당금 차트에서 특정 월을 클릭하면, 해당 월에 배당이 지급되는 종목의 상세 내역과 **'지급 완료' 또는 '예정' 상태**를 확인할 수 있습니다.
