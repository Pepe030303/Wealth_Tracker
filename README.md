...
-   **섹터 비중**: 보유 자산이 어떤 산업 섹터에 분포되어 있는지 시각적인 트리맵 차트로 보여줍니다.
-   **월별 배당금 현황**: 미래에 수령할 월별 배당금 총액을 예측하여 막대그래프로 보여줍니다. (과거 패턴 기반 예측 포함)
...
-   **배당금 분석 페이지 (`/dividends`)**:
    -   **연간 배당 흐름 예측**: 과거 5년치 배당 이력을 심층 분석하여 아직 공시되지 않은 미래의 배당까지 예측합니다. **월배당, 특별배당 등 복잡한 지급 패턴을 자동으로 반영**하여 현재 연도 12개월 전체의 배당금 흐름을 직관적으로 보여줍니다.
    -   **지능형 데이터 통합**: 월별 배당금은 ① DB의 실제 지급 완료 내역, ② API의 확정된 미래 배당, ③ 과거 패턴 기반의 예측 데이터를 우선순위에 따라 통합하여 가장 정확하고 합리적인 수치를 제공합니다.
    -   **인터랙티브 차트**: 월별 배당금 차트에서 특정 월을 클릭하면, 상세 내역과 함께 **예측된 배당락일** 정보를 확인할 수 있습니다.

-   **배당금 입금 내역 (`/dividends/history`)**:
    -   배당락일(Ex-Dividend Date) 기준 계산: 백그라운드 작업이 각 종목의 배당 정보를 가져와, **배당락일** 시점의 보유 수량을 기준으로 실제 받을 배당금을 정확히 계산하여 기록합니다.
    -   과거에 실제로 지급된(또는 지급될 예정인) 배당금 기록을 정확한 **지급일**과 **배당락일**과 함께 보여줍니다.
