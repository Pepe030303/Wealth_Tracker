...
### ✨ 보유 종목 관리 (`/holdings`)
- **자동 계산된 자산**: `/trades` 페이지의 기록을 바탕으로 자동 계산된 보유 종목 목록을 보여줍니다.
- **실시간 평가**: 각 종목의 로고(지연 로딩 적용)와 함께, 현재가와 평균 단가, 평가 금액과 총 투자 금액, 그리고 실시간 손익 및 수익률을 한눈에 보기 쉬운 간결한 레이아웃으로 제공합니다.

### 💸 배당금 분석
-   **배당금 분석 페이지 (`/dividends`)**:
    -   **종목별/월별 현황**: 보유 종목별(로고 포함) 연간 배당금과 월별 배당금 흐름을 시각적으로 제공합니다. 목록은 평가금액이 높은 순으로 정렬됩니다.
