...
-   **배당금 분석 페이지 (`/dividends`)**:
    -   **연간 배당 흐름 예측**: 과거 배당 이력을 분석하여 아직 공시되지 않은 미래의 배당까지 예측합니다. 이를 통해 사용자는 **현재 연도 12개월 전체의 배당금 흐름**을 직관적인 막대그래프로 한눈에 파악할 수 있습니다.
    -   **지능형 데이터 통합**: 월별 배당금은 ① 데이터베이스에 기록된 실제 지급 완료 내역, ② API를 통해 확인된 확정된 미래 배당, ③ 과거 패턴 기반의 예측 데이터를 우선순위에 따라 통합하여 가장 정확하고 합리적인 수치를 제공합니다.
    -   **인터랙티브 차트**: 월별 배당금 차트에서 특정 월을 클릭하면, 해당 월에 배당이 지급되는 종목의 상세 내역을 확인할 수 있습니다. 이 때, 예측된 데이터의 경우 정보 아이콘(ℹ️)이 표시되어 사용자가 데이터의 성격을 명확히 인지할 수 있습니다.
