...
### Changed
- **배당 예측 엔진 고도화**:
  - **액면분할 보정**: 주식 분할 이력을 반영하여 과거 배당금을 현재 기준으로 환산 후 예측에 사용함으로써 정확도를 대폭 향상시켰습니다.
  - **예측 로직 개선**: 분기별 최근 배당 패턴을 분석하여 SCHD와 같은 종목의 월별 중복 예측 오류를 해결하고, TLT 등 특정 월 다중 배당 종목의 패턴도 정확히 반영하도록 수정했습니다.
- **UI/UX 개선**:
  - **대시보드 개편**: 비효율적인 '섹터 비중 그래프'를 제거하고, 평가금액 기준 상위 5개 종목을 보여주는 '보유 종목 요약' 리스트로 대체했습니다.
  - **정렬 기준 통일**: 보유 종목 및 배당 분석 페이지의 종목 리스트를 '현재 평가금액' 기준 내림차순으로 정렬하여 일관성을 확보했습니다.
  - **배당월 표시 최적화**: 월배당 종목은 '월배당' 뱃지로, 그 외에는 실제 배당 월만 간결하게 표시하도록 UI를 개선했습니다.
  - **차트 인터랙션 강화**: 배당 분석 페이지의 월별 차트에서 특정 월을 클릭하면 해당 막대가 시각적으로 강조되도록 하여 사용자 경험을 개선했습니다.

---

## [v1.7.0] - 2025-07-13
### Changed
- **배당 예측 정확도 향상**: 과거 배당 이력 분석 시, 월별로 가장 최근 데이터만 사용하도록 예측 엔진을 개선하여 SCHD 등 분기 배당 종목이 중복으로 예측되던 오류를 해결했습니다.
- **UI/UX 개선**:
  - **대시보드 개편**: 비효율적인 '섹터 비중 그래프'를 제거하고, 평가금액 기준 상위 5개 종목을 보여주는 '보유 종목 요약' 리스트로 대체했습니다.
  - **정렬 기준 통일**: 보유 종목 및 배당 분석 페이지의 종목 리스트를 '현재 평가금액' 기준 내림차순으로 정렬하여 일관성을 확보하고 포트폴리오의 핵심 자산을 쉽게 파악할 수 있도록 개선했습니다.
  - **배당월 표시 최적화**: 월배당 종목은 12개의 월을 모두 나열하는 대신 '월배당' 뱃지로 간결하게 표시하도록 변경하여 UI 가독성을 높였습니다.
  - **차트 인터랙션 강화**: 배당 분석 페이지의 월별 차트에서 특정 월을 클릭하면 해당 막대가 시각적으로 강조되도록 하여 사용자 경험을 개선했습니다.
...
## [v1.5.0] - 2025-07-13
### Changed
- **대시보드 차트 로직 개선**: 대시보드의 '섹터 비중' 차트 렌더링 오류를 수정하고, 월별 배당금 및 섹터 비중 차트 생성 로직을 재사용 가능한 공통 JavaScript 함수(`static/js/charts.js`)로 추출하여 코드 중복을 제거하고 안정성을 높였습니다.

### Fixed
- 대시보드 페이지에서 차트 데이터가 없거나 비정상적일 때 스크립트 오류가 발생하며 페이지 전체의 차트 렌더링이 실패하던 문제를 해결했습니다.

---

## [v1.4.1] - 2025-07-13
### Fixed
- **서버 런타임 오류 수정**: `utils.py`의 배당 예측 함수 내에서 변수명 오타로 인해 발생하던 `NameError`를 수정했습니다. 이로 인해 대시보드 및 배당 분석 페이지 접근 시 발생하던 500 에러가 해결되었습니다.

---

## [v1.4.0] - 2025-07-13
### Added
- **대시보드에 월별 배당금 차트 추가**: 대시보드 페이지에 월별 배당금 현황 차트를 추가하여 사용자가 핵심 정보를 한눈에 파악할 수 있도록 개선했습니다. (상세 보기 기능은 제외)

### Changed
- **배당 예측 로직 고도화**: 과거 5년치 배당 이력을 분석하여 미래 배당을 예측하는 로직을 전면 개편했습니다. 이를 통해 월배당 및 특별배당 종목(O, TLT 등)의 지급 패턴을 정확하게 반영합니다.
- **미래 배당락일 예측**: API에서 제공하지 않는 미래의 예측 배당락일을 과거 데이터의 평균치를 기반으로 계산하여 표시하도록 개선하여, 'N/A'로 표시되던 문제를 해결했습니다.
- **JavaScript 코드 리팩토링**: 대시보드와 배당 분석 페이지에서 중복되던 월별 배당 차트 렌더링 스크립트를 재사용 가능한 단일 함수(`static/js/charts.js`)로 추출하여 코드 중복을 제거하고 유지보수성을 향상시켰습니다.

---

## [v1.3.1] - 2025-07-13
### Fixed
- **서버 런타임 오류 수정**: `services/portfolio_service.py` 파일에서 `collections.defaultdict`를 import하지 않아 발생하던 `NameError`를 해결했습니다. 이로 인해 대시보드 및 배당 분석 페이지 접근 시 발생하던 500 에러가 수정되었습니다.

---

## [v1.3.0] - 2025-07-13
### Added
- **배당 예측 기능 도입**: 과거 배당 이력(지급 월, 일)을 분석하여 현재 연도의 미래 배당을 예측하는 로직을 추가했습니다. 이를 통해 공식 발표되지 않은 배당금까지 포함한 전체 연간 배당 흐름을 파악할 수 있습니다.

### Changed
- **통합된 월별 배당 뷰**: 배당 분석 페이지의 차트를 '지급 완료/예정'으로 분리하는 대신, '실제 데이터'와 '예측 데이터'를 통합하여 보여주는 단일 월별 배당금 차트로 변경했습니다. 사용자 경험을 단순화하고 연간 흐름을 더 직관적으로 파악할 수 있도록 개선했습니다.
- **데이터 소스 통합 로직 개선**: 서비스 계층에서 DB의 실제 지급 기록, API의 확정된 미래 기록, 예측 엔진의 추정 기록을 우선순위에 따라 지능적으로 통합하여 최종 배당 데이터를 생성하도록 로직을 고도화했습니다.
